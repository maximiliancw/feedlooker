...
class RSSCrawler:
    ...
    def __init__(self, max_depth=2):
        ...
        """
        self.visited_urls = set()  # Keeps track of visited URLs to prevent duplicate visits.
        self.rss_feeds = set()  # Stores found RSS feeds as a set of URLs.
        self.max_depth = max_depth  # Maximum depth for crawling.
        self.common_feed_paths = ['/rss', '/feed', '/feeds', '/atom.xml', '/rss.xml']  # Common feed paths.
    ...
    async def find_rss_links(self, html, base_url):
        ...
        for a_tag in soup.find_all('a', href=True):
            if any(keyword in a_tag['href'].lower() for keyword in ['rss', 'feed', 'atom']):
    ...
    async def negotiate_feed_content(self, session, base_url):
        ...
        """
        _, content = await self.fetch(session, base_url, accept_header='application/rss+xml')
        if content:
            try:
                ...
                if content_type == 'html':
                    ...
                    if depth < self.max_depth:
                        ...
                        for a_tag in soup.find_all('a', href=True):
                            ...
                            if '.' in parsed_link.path.split('/')[-1]:
                                continue

                            # Drop query parameters before following the link
                            link = urlunparse(parsed_link._replace(query=''))
